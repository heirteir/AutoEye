...
public class NoWeb extends Check {
    ...
    @Override public boolean check(AutoEyePlayer player) {
        if (player.isConnected() && !player.getLocationData().isOnPiston() && player.getTimeData().getLastVelocity().getAmount() == 0 && player.getLocationData().isChangedPos() && player.getTimeData().getLastFlying().getAmount() == 0 && !player.getLocationData().isClientOnGround() && player.getLocationData().isInWeb()) {
